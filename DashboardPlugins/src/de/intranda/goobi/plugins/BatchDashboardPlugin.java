...
public class BatchDashboardPlugin implements IDashboardPlugin {
	...
	private void loadBatchesInInterval() {
		...
		String batchDetails = "select bearbeitungsstatus as one, count(schritteid) as two from schritte left join prozesse on schritte.prozesseid = prozesse.prozesseid where prozesse.batchId = ? group by bearbeitungsstatus";

		Connection connection = null;
		try {
			connection = MySQLHelper.getInstance().getConnection();
			QueryRunner run = new QueryRunner();
			batchesInInterval = run.query(connection, batches.toString(),
			        new BeanListHandler<DashboardHelperBatch>(DashboardHelperBatch.class));
	...
	private void loadOverviewData() {
		...
		sb.append(
		        "(select x.completed as completed, x.batchId, null as inwork, null as locked from (select count(s.SchritteID) as completed, ");
		...
		sb.append("union all ");
		sb.append(
		        "(select null, y.batchId, y.inwork as inwork, null as locked from (select count(s.SchritteID) as inwork, b.id as batchId ");
		sb.append(
		        "from schritte s left join prozesse p on s.ProzesseID = p.ProzesseID join batches b on p.batchID = b.id where ");
		sb.append(
		        "s.BearbeitungsStatus = 2 or s.BearbeitungsStatus = 1  or s.BearbeitungsStatus = 4 group by b.id) y) ");
		sb.append("union all ");
		sb.append(
		        "(select null, z.batchId, null as inwork, z.inwork as locked from (select count(s.SchritteID) as inwork, b.id as batchId ");
		...
		try {
			...
			QueryRunner run = new QueryRunner();
			processesWithoutBatch = run.query(connection,
			        "SELECT count(ProzesseID) from prozesse WHERE batchID is null",
			        MySQLHelper.resultSetToIntegerHandler);
			processesWithErrors = run.query(connection,
			        "select count(distinct(ProzesseID)) from schritte where Bearbeitungsstatus = 4",
			        MySQLHelper.resultSetToIntegerHandler);
			totalProcesses = run.query(connection, "select count(ProzesseID) from prozesse",
			        MySQLHelper.resultSetToIntegerHandler);
			completedProcesses = run.query(connection,
			        "select count(ProzesseID) from prozesse where sortHelperStatus = ?",
